...
function initUI(){
  ...
  resetBtn.onclick     = ()=>{ if(confirm('Reset all?')) localStorage.clear(), location.reload(); };
...
function sampleCoverColor(target='all'){
  ...
  img.onload = ()=>{
    ...
    if(target==='bg' || (smartBg.checked && target==='all')){
...
function gatherSettings(){
  const o={};
  ['bg-color','bg-fit','sync-bg','smart-bg','bar-color','prog-text','sync-prog-text',
   'title-color','album-color','artist-color','title-size','album-size','artist-size','radius']
  .forEach(k=>{
    const el = document.getElementById(k.replace(/-/g,'-'));
    if(!el) return;
